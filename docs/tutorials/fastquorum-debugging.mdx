...
description: 'Using OpenSRE to diagnose and optimize UMI-based consensus sequencing'
---

This tutorial walks a bioinformatics engineer through real-time observability of the nf-core/fastquorum pipeline using OpenSRE's eBPF-powered monitoring. We simulate a small but realistic UMI-based duplex sequencing workflow on a single chromosome (chr17.fa), run it in a GitHub Codespace, and use OpenSRE to detect resource bottlenecks, identify redundant I/O, and explain why the pipeline completed in 1m 36s despite only 12 processes.

## What You'll Learn

- Connect a live Codespace to the OpenSRE sandbox
...
<Info>
**Why this matters:** fastquorum is complex (UMI grouping, consensus calling, dual alignment). Without OS-level visibility, engineers guess where time is spent. OpenSRE shows exactly which process is the bottleneck — no logs, no profiling flags.
</Info>
...
## 1. Login & Setup: OpenSRE Sandbox + GitHub Codespaces

We begin in a GitHub Codespace — a reproducible, cloud-based dev environment that mimics a local VM. OpenSRE's eBPF agent runs natively here and streams metrics to the OpenSRE Sandbox Dashboard (https://dev.sandbox.tracer.cloud) in real time.
...
<Steps>
  ...
  <Step title="Install OpenSRE (One-Liner with Dev Branch & User Token)">
  ...
  <Step title="Start OpenSRE Agent">
    ...
    *Fig 2: You will see something like this upon successful connection (Snapshot of tracer init command which connecting to tracer)*
    ...
    <Tip>
    With OpenSRE agent connected, input validated, and genome indexed, we now execute the full nf-core/fastquorum pipeline. No code changes are required — OpenSRE's eBPF hooks automatically detect nextflow launches, label processes, and stream OS-level metrics (CPU, RAM, I/O, syscalls) to your sandbox dashboard in real time.
    </Tip>
  </Step>
</Steps>
...
To eliminate I/O noise during the observed run, the genome index (BWA-MEM1, SAMtools FAIDX, and DICT) is pre-built locally and stored for reuse, ensuring clean, reproducible eBPF telemetry from OpenSRE.
</Accordion>
...
## 4. Live Visualization: OpenSRE Dashboard During Execution

With the nf-core/fastquorum pipeline launched and OpenSRE's eBPF agent actively streaming OS-level events, the OpenSRE Sandbox Dashboard becomes a real-time observability cockpit. No polling, no logs — just continuous, kernel-level telemetry delivered via WebSocket every 2 seconds.
...
<Info>
This compact summary is the first signal that OpenSRE has auto-detected the Nextflow executor and attached to all child processes — no `-with-trace` or config changes needed. The progress bar fills as tasks complete, and resource meters update in real time.
</Info>
...
<Tip>
This panel confirms the GitHub Codespaces environment: a 4-core, 16 GB VM with ample headroom. The cost meter at $0.00 reflects that this is a non-billable sandbox run, but in production (e.g., AWS EC2), OpenSRE would estimate hourly cost based on instance type and utilization.
</Tip>
...
In the fast-evolving landscape of bioinformatics, where pipelines demand precision amid mounting computational complexity, **OpenSRE emerges as an indispensable ally** for bioinformaticians seeking deeper, actionable insights without the burden of invasive instrumentation.

### Key Benefits

By harnessing **eBPF technology** at the operating system level, OpenSRE delivers:
...
OpenSRE excels in resource orchestration, spotlighting inefficiencies like redundant I/O in alignment steps or overprovisioned instances.
</Accordion>
...
For bioinformaticians juggling high-throughput NGS data, evolving dependencies, and the pressure to derive reproducible insights from vast datasets, **OpenSRE isn't just a monitoring tool — it's a superpower** that shifts focus from infrastructure headaches to scientific discovery, fostering scalable, cost-effective workflows that accelerate breakthroughs in genomics, proteomics, and beyond.

<Card title="Try OpenSRE Sandbox" icon="flask" href="https://tracer.bio">
  Dive into the OpenSRE sandbox today and experience how effortless observability can redefine your pipeline mastery.
</Card>
