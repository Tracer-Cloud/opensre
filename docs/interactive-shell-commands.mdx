...
lets you submit a different request. Run `opensre --skip-onboarding` to open
the shell without the startup picker. Type `/demo` to start it later.
...
Run `opensre` to sign in before this workflow is reachable. Every launch opens the demo picker — Escape continues to the prompt. Run `opensre --skip-onboarding` to bypass it. The recommended option analyzes CI/CD performance; see [CI/CD analytics demo](/cicd-analytics-demo).
