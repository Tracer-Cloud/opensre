...
| `/resume` | Recent session names with the focused session's timestamp and short ID |
| `/loops show` | Loop names with the focused loop's ID; Enter opens its report |
...
These setting and resource menus use compact, scrolling panels. The highlighted row is
...
rows; the header counter shows position in the full list. `/theme` saves only after selection;
