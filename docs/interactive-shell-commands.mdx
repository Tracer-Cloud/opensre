...
<Info>
  ...
  elevated slash commands. Default `/auto` is **high** (no default tool prompts).
  `/trust on` skips ordinary approval prompts for the rest of the session. Python
  execution, if enabled, still requires approval for every call. Outside a TTY,
...
| --- | --- |
| `high` (default) | `execute_python_code` (when enabled) and `stop_hosted_gateway`; other tools run immediately |
...
**With `/trust`:** `/trust on` skips ordinary `/auto` prompts. Python execution still asks on every call, showing the generated source and inputs with secrets redacted; it is blocked outside a TTY and in headless runners without an approval hook. Turn trust off to restore ordinary `/auto` prompts.
...
It does not skip the per-call approval for opted-in Python execution.
...
| Control | Role |
| --- | --- |
| `/auto` | Which **agent tool types** need ordinary approval (default **high** = none); enabled Python execution still asks |
| `/trust` | Session override that **skips** ordinary approval prompts when `/auto` (or an elevated slash command) would ask; Python execution still asks |
...
/auto high  # alpha default — no ordinary tool prompts
/trust on   # skip ordinary prompts for this session
