...
`/model` keeps its menu running: **View configuration** prints the current models
and returns to the menu. Leave with **Done** or Esc. In a child picker, Esc goes
...
**Bare `/model`:** in a terminal this opens a menu rather than printing the table. The menu stays open: **View configuration** prints the current models and hands you back to it, so leave with **Done** or Esc. **Change model**, **Restore provider defaults**, and **Configure tool-call model** close it once the change succeeds. Use `/model show` when you only want the table.
