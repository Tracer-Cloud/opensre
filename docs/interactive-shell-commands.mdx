...
Type `/` to open the command tray attached to the input. Browse with the arrow
keys and press Enter to run the highlighted suggestion, or Tab to complete it
and edit before running. Esc closes the tray
without clearing your draft or stopping running work. On narrow terminals, the
highlighted command's description appears below the list.

In choice panels that show **`/ search`**, press `/` to filter, then type part
of a name or identifier. Search is available in theme, provider/model,
session, loop, and integration service pickers. Use arrows to browse matches
and Enter to select. Backspace edits the query; Esc clears search first, then
Esc again returns or closes the picker. Digits are query text while searching;
otherwise the numbered shortcuts still select visible rows. Filtering never
applies a setting until you select a match.
...
| Key | Effect |
| --- | --- |
| **Up/Down** | Browse suggestions while the command tray is open; otherwise recall persisted command history (when enabled) |
| **Tab** | Complete the highlighted suggestion |
| **Enter** | Run the highlighted suggestion, or submit the draft when the tray is closed |
| **Esc** | Close the command tray first; otherwise stop running work or clear the draft |
...
| `/integrations` | View connections and details, verify, set up, or remove an integration |
| `/mcp` | View connected servers, connect, or disconnect |
| `/model` | Change model, view configuration, configure tool-call model, or restore defaults |
| `/theme` | Pick a colour theme from the installed list |
| `/choose` | Reopen the selection menu the agent queued |
| `/trust`, `/verbose` | on / off |
| `/history` | View saved commands, pause/resume recording, change retention, or clear history |
| `/resume` | Recent sessions with the name first, followed by ID and timestamp |
| `/loops show` | Loop IDs and names on each row; Enter opens its report |

`/model` keeps its menu running: **View configuration** opens read-only details
with wrapped values and arrow-key scrolling when needed. Enter or Esc returns to
**View configuration** without adding a table to the transcript. Leave the menu
with **Done** or Esc. In a child picker, Esc goes
back one step without applying a change.

These setting and resource menus use compact, scrolling panels. The range and
arrows in the header show which rows are visible and whether more are above or
below; use Up/Down to reach them. The highlighted row is
keyboard focus; a checkmark marks the current value. Arrow keys only move focus;
Enter or a numbered shortcut selects an option. Numbers address the currently visible
rows. `/theme` saves only after selection;
Esc leaves it unchanged. Use `/effort` separately to inspect or change reasoning effort.

`/trust` and `/verbose` start on the current value and close after applying a selection.
Trust affects only this session; verbose logging affects the current process.
History retention changes apply to this session and prune older saved commands.
Clearing history and removing connections require a separate confirmation that starts
on **Cancel**. Esc from connection removal returns to the service picker.
Read-only connection and history views use detail panels; Enter or Esc returns to
the preceding menu. Explicit commands such as `/integrations list` retain printable output.
...
**Bare `/model`:** in a terminal this opens a menu rather than printing the table. **View configuration** opens read-only details; Enter or Esc returns to the menu. Leave with **Done** or Esc. **Change model**, **Restore provider defaults**, and **Configure tool-call model** close it once the change succeeds. Use `/model show` when you only want the table.
...
Sign in to a personal OpenSRE account, inspect the local login, show remaining hosted credits (`/account credits` or `/credits`, same as `opensre credits`), open the credits and top-up page (`/account usage`, opens your browser; from a chat it replies with the link), or sign out. Same as `opensre account`. The browser opens the OpenSRE sign-up page, where you can use email or any enabled sign-in provider. The shell starts only for an `active` account state. `/account logout` revokes the saved session, invalidates the hosted-model route, and closes the shell before another turn. If `OPENSRE_ACCOUNT_TOKEN` is set in the environment, it overrides the token saved by login until you unset it.
...
Show remaining OpenSRE hosted credits for the signed-in account. Same as `opensre credits` and `/account credits`. Requires an active login. A fetch or auth failure is reported as unavailable — it is never shown as a zero balance.
