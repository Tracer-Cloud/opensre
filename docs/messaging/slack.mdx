...
| `slack_proactive_message_history` | Read your send/suppress judgement ledger | None | No |
...
After a completed Slack turn is persisted, OpenSRE reviews that interaction in
the background against `MASTER_JUDGEMENT.md`. It may read up to 20 messages from
the originating thread, then either posts one evidence-backed follow-up to that
same thread or remains silent. It does not poll Slack or choose another channel.

Ask “show my recent proactive-message decisions” to retrieve the durable
send/suppress ledger, including delivery status and the Slack message timestamp.
The ledger and judgement cursor live under your scoped OpenSRE user context in
`proactive_messages/`.
