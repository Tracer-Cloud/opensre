...
<Steps>
  ...
  <Step title="Setup">
    Run first-run setup to create or sign in to your OpenSRE account. From a source checkout, prefix commands in this and the next step with `uv run` (for example, `uv run opensre setup`). To update a source checkout, use `git pull --ff-only` followed by `uv sync --frozen --extra dev` instead of `opensre update`.
