...
See [Windows](/environments/windows-local).
...
- **`opensre` is not found**: Open a new terminal, or add the bin directory printed by the installer to `PATH`.
- **The Linux binary does not start**: Prebuilt binaries require glibc 2.35 or newer; use a supported distribution or the [Docker guide](/environments/docker).
