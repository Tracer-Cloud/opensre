...
<Steps>
  <Step title="Install OpenSRE" id="install-opensre">
    <Tabs>
      <Tab title="macOS" id="install-opensre-macos">
      ...
      <Tab title="Linux" id="install-opensre-linux">
        Choose one install method. The one-line installer tracks the latest stable build from `main`.

        Prebuilt Linux binaries require **glibc 2.35+** (Ubuntu 22.04+ or a comparable distribution) and do not run on Alpine Linux. On an older or musl-based system, [install from source](/environments/linux-local#binary-compatibility) instead.
      ...
      <Tab title="Windows" id="install-opensre-windows">
        ```powershell
    ...
    Before installing, check the [supported platforms and architectures](https://github.com/Tracer-Cloud/opensre/blob/main/SETUP.md#supported-platforms-and-architectures) table for release availability and CI coverage, including the Windows arm64 limitation.

  </Step>

  <Step title="Get started">
    Run `opensre` to create or sign in to your OpenSRE account and start chatting. From a source checkout, prefix commands in this and the next step with `uv run` (for example, `uv run opensre`). To update a source checkout, use `git pull --ff-only` followed by `uv sync --frozen --extra dev` instead of `opensre update`.
