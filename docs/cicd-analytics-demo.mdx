...
1. **Scan.** OpenSRE finds git repositories under your home directory, counts commits in the last 30 days and uncommitted files, notes which have GitHub Actions workflows, and draws the activity chart. Clones of the same repository are folded into one row.
2. **Pick.** It offers the most active repositories that have CI configured, plus an open-source example, and asks which one to analyze.
