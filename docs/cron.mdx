...
# Every 30 minutes: fix one open security or quality finding and open a PR → Slack
...
| `--owner` | Cond. | GitHub repository owner; required for the `reporting-github-ci-failures` and `fixing-github-security-alerts` recurring skills |
| `--repo` | Cond. | GitHub repository name; required for the `reporting-github-ci-failures` and `fixing-github-security-alerts` recurring skills |
...
| `--workspace` | No | Local checkout for `fixing-github-security-alerts`; defaults to `CODING_WORKSPACE`, then the scheduler's working directory |
...
| `recurring_skill` | Runs a named, revision-pinned recurring skill. `delivering-morning-briefings` preserves its optional city; `reporting-github-ci-failures` requires repository scope and can be narrowed to a branch or PR; `fixing-github-security-alerts` requires repository scope, fixes one open finding per tick from a fresh linked worktree of the checkout, and opens a PR (see [GitHub security fix](/github-security-fix#scheduled-sweeps)). |
