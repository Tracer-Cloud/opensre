...
Show the number of durably admitted runs still pending, when the oldest one was
admitted, and its current age. Pending work remains visible across scheduler
restarts and while a task is paused or disabled. Use `--json` for structured
output suitable for monitoring and runbooks. JSON output includes `status: ok`;
if the task store cannot be read, the command exits unsuccessfully and reports
`status: unknown` instead of returning a misleading empty backlog.
