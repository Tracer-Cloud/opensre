...
opensre cron add --kind recurring_skill --skill delivering-morning-briefings \
  --cron "0 8 * * mon-fri" --tz Asia/Kolkata \
...
`0` is Sunday). Prefer `mon-fri` for weekdays to avoid confusion.

Older weekday loops may still contain `1-5`, which runs Tuesday–Saturday.
Check `/loops next <loop_id>`. To correct one, record its settings, stop it with
`/loops stop <loop_id>`, then recreate it through the original scheduling command
with the same settings and `--cron "0 9 * * mon-fri"` for weekdays at 09:00.
For `/loops add`, you can also use `--time 09:00 --weekdays`.
For an old Monday-only schedule, replace the final `1` with `mon` when recreating it.
Delete the old loop with `/loops delete <loop_id>` after verifying the replacement.
Existing schedules are not changed automatically. For a CI reliability loop,
delete the old loop before scheduling it again; that flow reuses existing loops.
...
- **Durable pending count** and **oldest pending age**: `pending` rows and the
  elapsed time since their durable admission. Read them with `opensre cron
  status`; the benchmark reads the same stores directly.
