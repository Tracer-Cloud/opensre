...
| `OPENSRE_ACCOUNT_LLM_MODEL` | `gpt-5.6-sol` | Model-family hint for a non-interactive hosted gateway using the account LLM route; override only when the web app's hosted model changes family |
...
| Variable | Default | Description |
| --- | --- | --- |
| `OPENSRE_HOME` | `~/.opensre` | Host root for scheduler files, gateway state, local configuration, and sessions when no organization context root is set |
...
| `OPENSRE_CONTEXT_ROOT` |  | Mounted organization-scoped root for context and sessions; requires `ORGANIZATION_ID` |
...
## Gateway and scheduler runtime

| Variable | Default | Description |
| --- | --- | --- |
| `MODE` | `web` | Container entrypoint: `web`, `gateway`, or `scheduler` |
| `PORT` | `8000` | HTTP port for the web surface |
| `OPENSRE_HOME` | `~/.opensre` | Host root for scheduler files, gateway state, local configuration, and sessions when `OPENSRE_CONTEXT_ROOT` is unset |
| `DATABASE_URL` |  | Postgres connection for gateway records shared across replicas; required for Slack Events API unless the single-replica local-dedup escape hatch is enabled |
| `OPENSRE_SIZE_PROFILE` | `SMALL` | Deployment size profile; default turn caps are `SMALL=1`, `MEDIUM=2`, `LARGE=4` |
| `OPENSRE_MAX_CONCURRENT_TURNS` | size-profile limit | Override the process-wide concurrent-turn cap with a positive integer |
| `OPENSRE_SCHEDULER_MAX_CONCURRENT_RUNS` | `2` | Maximum scheduled callbacks executing at once; agent turns also need a process-wide turn permit |
| `OPENSRE_GATEWAY_HOST_SCHEDULER` | `1` | Run the scheduler inside the gateway; set `0` when a dedicated `MODE=scheduler` service owns it |
| `OPENSRE_SESSION_FILE_LOCK` | `0` | Serialize session-file writes with an OS lock; a shared mount is still required across processes |
...
| `SLACK_GATEWAY_MAX_CONCURRENT` | size-profile limit | Parallel Slack turns before the process-wide cap is applied |
...
| `DISCORD_GATEWAY_MAX_CONCURRENT` | size-profile limit | Parallel Discord turns before the process-wide cap is applied |
...
| `TELEGRAM_GATEWAY_MAX_CONCURRENT` | size-profile limit | Parallel Telegram turns before the process-wide cap is applied |
| `BUZZ_PRIVATE_KEY` |  | Nostr private key used by the Buzz integration and gateway |
| `BUZZ_RELAY_URL` | `http://localhost:3000` | Buzz relay URL |
| `BUZZ_DEFAULT_CHANNEL` |  | Default Buzz channel UUID for delivery |
| `BUZZ_AUTH_TAG` |  | Optional NIP-OA owner-attestation JSON |
| `BUZZ_PATH` | `buzz` | Path to the Buzz CLI |
| `BUZZ_ALLOWED_PUBKEYS` |  | Comma-separated 64-character hex pubkeys allowed to talk to the gateway |
| `BUZZ_GATEWAY_MAX_CONCURRENT` | `4` | Parallel Buzz turns before the process-wide cap is applied |
