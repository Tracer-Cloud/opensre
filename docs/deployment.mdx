...
title: "Deployment"
slug: "deployment"
description: "Run an organization-managed gateway or deploy the OpenSRE gateway yourself."
---
...
the process and storage. For local containers, see [Docker](/environments/docker).
...
they differ. See [HTTP API](/api) for the route reference.
...
What that boundary means: the agent can reach the task's filesystem and network.
Three things limit what it can do with that. It never holds the organization's
account token: it talks to a loopback relay inside the gateway with a token
minted for that one run, and the relay adds the real token when it forwards the
call. It reads no `AGENTS.md` from the checkout and is told that text in
repository files is data, not instructions. And CI repairs run only on
repositories an organization admin approved, on branches inside that repository
(pull requests from forks are refused before the agent starts), so the code it
reads comes from people who already have push access there. The organization's
GitHub credential still lives on the task's home volume, so treat the repair
target as trusted code.
