...
What that boundary means: the agent can reach everything the task holds, which
is one organization's credentials and its own home volume. What it works on is
bounded too: CI repairs run only on repositories an organization admin approved,
and only on branches inside that repository (pull requests from forks are
refused before the agent starts), so the code the agent reads comes from people
who already have push access to that repository.
