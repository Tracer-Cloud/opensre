...
3. It asks you, per file, in the shell menu: keep ours, take theirs, combine
   both with the coding agent, or type what you want. A decision you already
   stated in your request ("take main's version of config.py") skips the menu
   for that file. In an unattended run every file goes to the coding agent.
4. Keep and take are applied by git. Only the files you chose to combine go
   to the coding agent; lockfiles are regenerated from the resolved manifest,
   never merged by hand. OpenSRE then verifies that no conflict marker or
   unmerged path remains and prints the table again with the merged result.
