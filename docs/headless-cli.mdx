...
Files are treated as untrusted context, not operator instructions. Each file
may contain up to 64 KiB, with a combined limit of 128 KiB. Empty, binary, and
non-UTF-8 files are rejected before the agent starts. The combined limit also
applies after JSON encoding, so content dominated by control characters,
quotes, or backslashes may reach it earlier.
