...
option number or your own answer:
...
Use `--ephemeral` for scripts or sensitive one-off requests that should not
write conversation history or print a resumable session ID:
...
label, options, and whether it accepts multiple selections. The `error` value
is either `null` or an object with `message` and `suggestion`.
