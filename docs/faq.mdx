...
<AccordionGroup>
  ...
  <Accordion title="Which deployment path should I use?">
    ...
    tools you want OpenSRE to query). For an organization-managed runtime, use
    [Enterprise setup](/install-enterprise) at
    [app.opensre.com](https://app.opensre.com). Hosted organization-wide
    connectors are still coming soon, so integrations remain local today.

    Managed and self-hosted gateway options are covered in
    [Deployment](/deployment).
  </Accordion>

  <Accordion title="Can I self-host OpenSRE?">
    Yes. Deploy the repository Docker image in `web`, `gateway`, or `scheduler`
    mode. Set `LLM_PROVIDER` and the
