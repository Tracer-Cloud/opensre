...
def test_is_available_when_connection_verified() -> None:
...
def test_run_returns_unavailable_without_api_url() -> None:
    tool = PrefectWorkerHealthTool()
...
def test_run_identifies_unhealthy_pools() -> None:
    tool = PrefectWorkerHealthTool()
    pools = [
...
def test_run_empty_work_pools() -> None:
    tool = PrefectWorkerHealthTool()
    mock_client = MagicMock()
    mock_client.__enter__ = MagicMock(return_value=mock_client)
    mock_client.__exit__ = MagicMock(return_value=False)
...
def test_run_fetches_workers_when_pool_name_provided() -> None:
    tool = PrefectWorkerHealthTool()
    workers = [
...
def test_run_skips_worker_fetch_without_pool_name() -> None:
    tool = PrefectWorkerHealthTool()
    mock_client = MagicMock()
    mock_client.__enter__ = MagicMock(return_value=mock_client)
    mock_client.__exit__ = MagicMock(return_value=False)
