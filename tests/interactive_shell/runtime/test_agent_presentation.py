...
def test_credit_wall_queues_a_menu_with_the_ways_out() -> None:
    """Like Claude Code's inline billing prompt: pick top-up or switch provider."""
    # Arrange: a real shell session (has a terminal) and the console event sink.
    from surfaces.interactive_shell.session import Session

    session = Session()
    sink = ConsoleAgentEventSink(session=session, spinner=MagicMock(), console=_RecordingConsole())  # type: ignore[arg-type]
    ...
    asyncio.run(
        sink(
            AgentEvent(
                type="turn_error",
                error=OpenSRECreditsExhaustedError(
                    "OpenSRE credits exhausted", upgrade_url="https://app.opensre.com/usage"
                ),
            )
        )
    )

    # Assert: the menu is queued and each option maps to a slash command, not a model turn.
    pending = session.pending_user_choice
    assert pending is not None
    assert pending.title == ap.CREDITS_MENU_TITLE
    assert pending.commands == {
        ap.CREDITS_OPTION_TOP_UP: "/account usage",
        ap.CREDITS_OPTION_SWITCH: "/model",
    }
    assert session.terminal.pending_prompt_default == "/choose"


def test_other_turn_errors_queue_no_menu() -> None:
    from surfaces.interactive_shell.session import Session

    session = Session()
    sink = ConsoleAgentEventSink(session=session, spinner=MagicMock(), console=_RecordingConsole())  # type: ignore[arg-type]

    asyncio.run(sink(AgentEvent(type="turn_error", error=RuntimeError("boom"))))
