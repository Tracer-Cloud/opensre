...
CONFIG = {
    "airflow_api_url": os.getenv("AIRFLOW_API_URL", "http://54.224.174.61:8080/api/v2"),
    "dag_id": os.getenv("AIRFLOW_DAG_ID", "upstream_downstream_pipeline_airflow"),
    "log_group": os.getenv("AIRFLOW_LOG_GROUP", "/ecs/tracer-airflow"),
    "correlation_id": os.getenv("AIRFLOW_CORRELATION_ID", "direct-20260131-234814"),
    "s3_bucket": os.getenv(
        "AIRFLOW_S3_BUCKET",
        "tracerairflowecsfargate-databucketslandingbucket03-j0q8arivcr9t",
    ),
    "s3_key": os.getenv("AIRFLOW_S3_KEY", "ingested/20260131-234814/data.json"),
    "audit_key": os.getenv("AIRFLOW_AUDIT_KEY", "audit/direct-20260131-234814.json"),
}


def _airflow_base_url() -> str:
    api_url = CONFIG["airflow_api_url"].rstrip("/")
    if "/api/" in api_url:
