...
def mock_aws_credentials():
    with patch("integrations.aws.lambda_client.require_aws_credentials", return_value=None):
        yield


@pytest.fixture
def mock_lambda_client():
    with patch("integrations.aws.lambda_client._get_lambda_client") as mock:
        client = MagicMock()
...
def mock_logs_client():
    with patch("integrations.aws.lambda_client._get_cloudwatch_logs_client") as mock:
        client = MagicMock()
        mock.return_value = client
        yield client
