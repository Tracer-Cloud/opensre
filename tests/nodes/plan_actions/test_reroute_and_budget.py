...
def test_plan_actions_uses_precomputed_available_sources(monkeypatch: pytest.MonkeyPatch):
    """Plan actions should skip detect_sources when available_sources is precomputed."""
    ...
    def _fail_detect_sources(*_args, **_kwargs):
        raise AssertionError("detect_sources should not be called when available_sources is provided")

    monkeypatch.setattr(plan_actions_module, "detect_sources", _fail_detect_sources)
    monkeypatch.setattr(plan_actions_module, "get_available_actions", lambda: [])
    ...
    (
        ...
        rerouted,
        reroute_reason,
    ) = plan_actions_module.plan_actions(
        input_data=input_data,
        plan_model=type("DummyPlan", (), {}),
        available_sources={"cloudwatch": {"log_group": "/aws/lambda/test"}},
    )

    assert plan is None
    assert available_sources == {"cloudwatch": {"log_group": "/aws/lambda/test"}}
