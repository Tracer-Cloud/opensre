...
```bash
python -m tests.synthetic_testing.rds_postgres.run_suite --mock-grafana
```

Run a single scenario:

```bash
python -m tests.synthetic_testing.rds_postgres.run_suite --scenario 006-replication-lag-cpu-redherring --mock-grafana
```

Print JSON results:

```bash
python -m tests.synthetic_testing.rds_postgres.run_suite --mock-grafana --json
```
