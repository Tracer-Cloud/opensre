...
def should_continue_investigation(state: InvestigationState) -> str:
    ...
    try:
        ...
        console.print(f"  [dim]  confidence={confidence:.0%}, validity={validity_score:.0%}, loop={loop_count}/{max_loops}, recommendations={len(investigation_recommendations)}[/]")
        ...
        should_loop = (confidence < confidence_threshold or validity_score < validity_threshold) and bool(investigation_recommendations)

        if should_loop:
            console.print("  [cyan]→ Looping back to hypothesis generation (low confidence/validity with recommendations)[/]")
            return "generate_hypotheses"

        console.print("  [green]→ Proceeding to publish findings (thresholds met or no recommendations)[/]")
        return "publish_findings"
