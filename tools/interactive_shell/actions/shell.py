...
shell_run_tool = RegisteredTool(
    ...
    description=(
        ...
        "per step when a step consumes the previous step's output. Each call starts a new "
        "host-shell process in the task workspace, so directory changes and other shell "
        "state do not persist across calls; use `cd path && command` for one call, or start "
        "a new task when work should continue in another workspace. When the user asks for "
