...
def run_local_llm_setup() -> int:
    ...
    # 4. Model recommendation + user override
    ...
    # Store Ollama host in keyring (used by resolve_llm_api_key at runtime)
    save_llm_api_key(provider.api_key_env, host)
    # Write OLLAMA_HOST and OLLAMA_MODEL to .env explicitly (host is not a secret but needed at runtime)
    ...
    _console.print("\nNext steps:")
    _console.print("  [bold]opensre investigate[/bold]  — run an RCA investigation")
