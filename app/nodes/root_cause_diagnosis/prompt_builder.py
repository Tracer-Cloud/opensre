...
def build_diagnosis_prompt(
    ...
    database_directive = _build_database_directive()
...
def _build_database_directive() -> str:
...
- Connection pool leaks (exhausting `max_connections`) are a `resource_exhaustion` root cause. Leaked or idle sessions primarily create connection pressure, not CPU load by themselves. If connections are near 100%, the root cause is connection exhaustion, not CPU saturation. If connections are near 100% AND CPU is near 100%, look for a single shared root cause: a connection pool leak that leaves active scan-heavy queries running or held open, causing CPU from active query workload and connection exhaustion at the same time (do not treat them as two independent problems).
