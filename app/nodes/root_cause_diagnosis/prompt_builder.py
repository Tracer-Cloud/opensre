...
- Replication lag: If a massive write-heavy workload on the primary generates WAL faster than the read replica can replay it, resulting in ReplicaLag spikes, the root cause is `resource_exhaustion` driven by the write workload on the primary. Watch out for red herrings: if concurrent analytics queries cause high CPU, do NOT classify the root cause as CPU-driven if the actual failing metric (like ReplicaLag) is driven by the write-heavy workload. Treat the CPU spike as an unrelated confounder/red herring and explicitly describe it as a red herring (i.e., not the root cause) in the final answer. Do not treat it as a second root cause or a contributing cause. Explicitly state that the analytics SELECT is not the root cause when the evidence shows it is causally independent from WAL generation. If the `ReplicaLag` metric is missing, infer lag from RDS events (e.g., 'exceeded 900s') and high `TransactionLogsGeneration`. This inference rule applies to single-fault replication scenarios and should not override compositional fault evaluation where multiple independent causes are present.
- Compositional Faults: If two completely independent workloads cause two separate faults simultaneously (e.g., CPU saturation from an analytics SELECT AND storage exhaustion from an audit_log INSERT), explicitly identify BOTH as independent root causes (do not merge them into a single IOPS fault). You MUST explicitly state they are two independent, coincidental faults. Provide evidence for both the analytics query and the audit_log query. You MUST explicitly include the keyword `audit_log` in ROOT_CAUSE when storage exhaustion is driven by audit logging. Do NOT replace it with generic terms like logs, transaction logs, temporary tables, or ingestion. The correct mechanism name must be used: `audit_log`. Use `resource_exhaustion` as ROOT_CAUSE_CATEGORY and describe both causes clearly in ROOT_CAUSE (e.g., "Two independent root causes: ..."). Trace each causal chain separately in CAUSAL_CHAIN. You MUST explicitly state that connection growth is a symptom of blocked writers, not connection exhaustion. If replication lag signals (e.g. ReplicaLag, WAL pressure, or replication delay indicators) are present or strongly implied by write-heavy workload evidence, explicitly state that ReplicaLag is a downstream symptom of the write/WAL burst and not an independent root cause. If such signals are not present in the evidence, do not fabricate them. Do not mention ReplicaLag at all if there is no evidence or strong implication of replication delay. NEVER diagnose `connection_exhaustion` as a root cause when connections spike due to a blocked write queue.
