...
description: >-
  Sets up ongoing local monitoring of one repository's open pull requests, automatically editing, testing,
  and pushing fixes for failing GitHub Actions checks. Offers a disposable private-repository demonstration
  when no target PR was supplied. Use for recurring PR repair or the local CI onboarding demo. A one-off
  PR repair belongs to fixing-github-ci.
...
metadata:
  ...
  requires:
  - GitHub authentication with write access to the watched repository.
  - An installed and authenticated coding agent.
  - A matching local checkout available to the scheduler host.
  - A local scheduler supporting agent-mode loops and the fix_github_pr_ci tool.
  - For a disposable demo, authorization to create and clean up its private repository and workflow.
  version: '4.0'
includes:
- common/ask_once.md
---
