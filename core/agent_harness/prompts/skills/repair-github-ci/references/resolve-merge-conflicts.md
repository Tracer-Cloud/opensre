...
2. Asks the user, per conflicted file, through the shell menu: keep ours,
   take theirs, combine with the coding agent, or free text. When the result
   says `menu: queued`, end the turn; the answers arrive as the next user
   message and the next call with no arguments reads them itself. Pass
   `decisions` only for choices the user already stated in words.
